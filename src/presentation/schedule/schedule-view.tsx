...
function TaskModal({locale,selectedKey,todayKey,options,initial,onClose,onSubmit}:{locale:Locale;selectedKey:string;todayKey:string;options:Option[];initial?:Item;onClose:()=>void;onSubmit:FormAction}) {
  ...
  return <div className={styles.modalLayer} id="taskModal"><button className={styles.backdrop} aria-label={locale==="ko"?"닫기":"Close"} onClick={onClose}/><section className={`${styles.modal} ${styles.taskModal}`} role="dialog" aria-modal="true" aria-labelledby="taskDialogTitle"><header><div><h2 id="taskDialogTitle">{initial?(locale==="ko"?"작업 수정":"Edit task"):t.newTitle}</h2><p>{t.newHelp}</p></div><button className={styles.iconButton} type="button" onClick={onClose}><Icon name="close"/></button></header><div className={styles.modalBody}>
    ...
    <div className={styles.field} data-select-root><span>{repeat==="none"?t.scheduleField:(locale==="ko"?"반복 시작일":"Repeat starts")}</span><button className={`${styles.selectTrigger} task-schedule-trigger modal-schedule-trigger`} type="button" aria-label={repeat==="none"?t.scheduleField:(locale==="ko"?"반복 시작일":"Repeat starts")} onClick={event=>{setScheduleAnchor(event.currentTarget);setOpen(open==="schedule"?null:"schedule");}}><span><Icon name="calendar"/><b className="schedule-value">{scheduleLabel}</b></span><Icon name="chevronDown"/></button>{open==="schedule"&&<ScheduleDateMenu locale={locale} todayKey={todayKey} selected={scheduleDates} singleDate={repeat!=="none"} trigger={scheduleAnchor} onClose={()=>setOpen(null)} onChange={dates=>{setScheduleDates(dates);if(dates.length>1)setRepeat("none");setOpen(null);}}/>}{repeatNeedsDate&&<small className="field-guidance is-warning">{locale==="ko"?"반복 작업은 시작일 한 개를 선택해야 해요.":"Choose one start date for a repeating task."}</small>}</div>
    <div className={styles.field} data-select-root><span>{t.repeat}</span><button ref={repeatTrigger} className={styles.selectTrigger} type="button" aria-label={t.repeat} disabled={multipleDates} onClick={()=>openFloating("repeat",repeatTrigger.current)}><span><b className="repeat-label">{repeat==="none"?t.noRepeat:t[repeat as "daily"|"weekly"|"monthly"]}</b></span><Icon name="chevronDown"/></button>{open==="repeat"&&<div className={`${styles.selectMenu} is-floating repeat-menu`} style={floatingRect??undefined}>{(["none","daily","weekly","monthly"] as const).map(value=><button type="button" key={value} onClick={()=>{setRepeat(value);setOpen(null);}}>{value==="none"?t.noRepeat:t[value]}</button>)}</div>}{repeatConflict&&<small className="field-guidance is-warning">{locale==="ko"?"반복을 유지하려면 날짜를 하나만 남겨주세요.":"Keep one date to continue using repeat."}</small>}</div>
...
function ScheduleDateMenu({locale,todayKey,selected,singleDate,trigger,onClose,onChange}:{locale:Locale;todayKey:string;selected:string[];singleDate:boolean;trigger:HTMLButtonElement|null;onClose:()=>void;onChange:(dates:string[])=>void}) {
  ...
  const [draft,setDraft]=useState(selected),[mode,setMode]=useState<"none"|"date">(selected.length?"date":"none"),[calendarOpen,setCalendarOpen]=useState(selected.some(date=>date!==todayKey&&date!==tomorrowKey)),popover=useRef<HTMLDivElement|null>(null),[position,setPosition]=useState<{left:number;top:number}|null>(null);
  const monthKey=(draft[0]??todayKey).slice(0,7), [year,month]=monthKey.split("-").map(Number);
  ...
  },[trigger,mode,calendarOpen,draft.length]);
  return <div ref={popover} className="schedule-popover popover" data-schedule-popover onPointerDown={event=>event.stopPropagation()} onClick={event=>event.stopPropagation()} style={position??undefined}>
    ...
    <div className="schedule-popover-options"><button className={mode==="none"?"is-active":""} type="button" onClick={()=>{setMode("none");setDraft([]);setCalendarOpen(false);}}><Icon name="calendar-off"/><span>{locale==="ko"?"일정 없음":"No date"}</span></button><button className={draft.includes(todayKey)?"is-active":""} type="button" onClick={()=>toggle(todayKey)}><Icon name="calendar"/><span>{locale==="ko"?"오늘":"Today"}</span></button><button className={draft.includes(tomorrowKey)?"is-active":""} type="button" onClick={()=>toggle(tomorrowKey)}><Icon name="calendar"/><span>{locale==="ko"?"내일":"Tomorrow"}</span></button><button className={calendarOpen?"is-active":""} type="button" onClick={()=>{setMode("date");setCalendarOpen(value=>!value);}}><Icon name="calendar-days"/><span>{locale==="ko"?"날짜 선택":"Choose dates"}</span></button></div>
    {mode==="date"&&calendarOpen&&<div className="schedule-popover-calendar"><header><button className={styles.iconButton} type="button"><Icon name="chevronLeft"/></button><strong>{label}</strong><button className={styles.iconButton} type="button"><Icon name="chevronRight"/></button></header><div className={styles.calendarGrid}>{weekdays.map((day,index)=><span key={`${day}-${index}`}>{day}</span>)}{Array.from({length:offset},(_,index)=><span key={`blank-${index}`}/>)}{Array.from({length:count},(_,index)=>{const key=`${monthKey}-${String(index+1).padStart(2,"0")}`;return <button type="button" key={key} className={draft.includes(key)?styles.selected:""} onClick={()=>toggle(key)}>{index+1}</button>;})}</div><div className="schedule-selection-row"><p className="schedule-selection-summary">{singleDate?(locale==="ko"?"반복 시작일은 한 날짜만 선택할 수 있어요.":"A repeating task can have one start date."):(draft.length?(locale==="ko"?`${draft.length}개 날짜 선택`:`${draft.length} ${draft.length===1?"date":"dates"} selected`):(locale==="ko"?"날짜를 여러 개 선택할 수 있어요.":"You can select multiple dates."))}</p>{!singleDate&&draft.length>1&&<button className="schedule-selection-clear" type="button" onClick={()=>setDraft([])}><Icon name="undo" size={10}/><span>{locale==="ko"?"선택 초기화":"Clear selection"}</span></button>}</div></div>}
