...
export function ApprovedSchedulePresentation(props:{
  ...
  const selectDate=(date:string)=>{
    ...
    else router.push(`/?date=${date}&month=${date.slice(0,7)}`);
  };
  ...
  return <div className={styles.root}><ApprovedIconSprite/><div className={styles.shell}>
    ...
    <main className={styles.content}>
      ...
      <div className={styles.scheduleGrid}><div>
        ...
        </article>)}</div>:<><div className={styles.taskList}/><div className={styles.empty}>{isToday?<button className={`${styles.button} ${styles.primary}`} type="button" onClick={()=>setTaskModal(true)}><Icon name="plus"/><span>{t.newTask}</span></button>:<><span className={styles.emptyIcon}><Icon name="moon"/></span><strong>{t.empty}</strong><small>{t.emptyHelp}</small></>}</div></>}
...
function TaskModal({locale,selectedKey,todayKey,options,initial,onClose,onSubmit}:{locale:Locale;selectedKey:string;todayKey:string;options:Option[];initial?:Item;onClose:()=>void;onSubmit:FormAction}) {
  ...
  return <div className={styles.modalLayer} id="taskModal"><button className={styles.backdrop} aria-label={locale==="ko"?"닫기":"Close"} onClick={onClose}/><section className={`${styles.modal} ${styles.taskModal}`} role="dialog" aria-modal="true" aria-labelledby="taskDialogTitle"><header><div><h2 id="taskDialogTitle">{initial?(locale==="ko"?"작업 수정":"Edit task"):t.newTitle}</h2><p>{t.newHelp}</p></div><button className={styles.iconButton} type="button" onClick={onClose}><Icon name="close"/></button></header><div className={styles.modalBody}>
    ...
    <div className={styles.field}><span>{t.repeat}</span><div className="schedule-options repeat-options">{(["none","daily","weekly","monthly"] as const).map(value=><button type="button" key={value} disabled={multipleDates&&value!=="none"} className={repeat===value?"is-active":""} onClick={()=>setRepeat(value)}>{value==="none"?t.noRepeat:t[value]}</button>)}</div>{repeatConflict&&<small className="field-guidance is-warning">{locale==="ko"?"반복을 유지하려면 날짜를 하나만 남겨주세요.":"Keep one date to continue using repeat."}</small>}</div>
...
function ScheduleDateMenu({locale,todayKey,selected,singleDate,onChange}:{locale:Locale;todayKey:string;selected:string[];singleDate:boolean;onChange:(dates:string[])=>void}) {
  const tomorrowDate=new Date(`${todayKey}T00:00:00Z`);tomorrowDate.setUTCDate(tomorrowDate.getUTCDate()+1);
  const tomorrowKey=tomorrowDate.toISOString().slice(0,10);
  const [calendarOpen,setCalendarOpen]=useState(selected.some(date=>date!==todayKey&&date!==tomorrowKey));
  const monthKey=(selected[0]??todayKey).slice(0,7), [year,month]=monthKey.split("-").map(Number);
  ...
  const toggle=(key:string)=>onChange(singleDate?[key]:selected.includes(key)?selected.filter(date=>date!==key):[...selected,key].sort());
  return <div className="inline-schedule-picker">
    <div className="schedule-options"><button className={!selected.length?"is-active":""} type="button" onClick={()=>{onChange([]);setCalendarOpen(false);}}><Icon name="calendar-off"/><span>{locale==="ko"?"없음":"None"}</span></button><button className={selected.includes(todayKey)?"is-active":""} type="button" onClick={()=>toggle(todayKey)}><Icon name="calendar"/><span>{locale==="ko"?"오늘":"Today"}</span></button><button className={selected.includes(tomorrowKey)?"is-active":""} type="button" onClick={()=>toggle(tomorrowKey)}><Icon name="calendar"/><span>{locale==="ko"?"내일":"Tomorrow"}</span></button><button className={calendarOpen?"is-active":""} type="button" onClick={()=>setCalendarOpen(value=>!value)}><Icon name="calendar-days"/><span>{locale==="ko"?"날짜 선택":"Choose dates"}</span></button></div>
    {calendarOpen&&<div className="date-picker"><header><span/><strong>{label}</strong><span/></header><div className={styles.calendarGrid}>{weekdays.map((day,index)=><span key={`${day}-${index}`}>{day}</span>)}{Array.from({length:offset},(_,index)=><span key={`blank-${index}`}/>)}{Array.from({length:count},(_,index)=>{const key=`${monthKey}-${String(index+1).padStart(2,"0")}`;return <button type="button" key={key} className={selected.includes(key)?styles.selected:""} onClick={()=>toggle(key)}>{index+1}</button>;})}</div><div className="schedule-selection-row"><p className="schedule-selection-summary">{singleDate?(locale==="ko"?"반복 시작일은 한 날짜만 선택할 수 있어요.":"A repeating task can have one start date."):(selected.length?(locale==="ko"?`${selected.length}개 날짜 선택`:`${selected.length} ${selected.length===1?"date":"dates"} selected`):(locale==="ko"?"날짜를 여러 개 선택할 수 있어요.":"You can select multiple dates."))}</p>{!singleDate&&selected.length>1&&<button className="schedule-selection-clear" type="button" onClick={()=>onChange([])}><Icon name="undo" size={10}/><span>{locale==="ko"?"선택 초기화":"Clear selection"}</span></button>}</div></div>}
  </div>;
}
