...
export default async function AreasPage({ searchParams }: { searchParams: Promise<{ area?: string }> }) {
  ...
  return <main className="wd-app"><AppNav/><section className="wd-main">
    <header className="wd-page-head"><div><span className="wd-eyebrow">{locale === "ko" ? "지속적으로 관리하는 범위" : "ONGOING RESPONSIBILITIES"}</span><h1>{locale === "ko" ? "영역" : "Areas"}</h1><span className="wd-muted">{locale === "ko" ? "꾸준히 관리할 습관과 관련 프로젝트를 한곳에 모아요." : "Keep ongoing habits and related projects together."}</span></div><NewAreaButton locale={locale}/></header>
    <div className="wd-workspace">
      <aside className="wd-rail"><div className="wd-rail-head"><button className="railCollapse" type="button" aria-label={locale === "ko" ? "영역 목록 접기" : "Collapse Areas"}>‹</button></div><nav>{areas.map(area => <Link scroll={false} className={area.id === selected?.id ? "is-active" : ""} href={`/areas?area=${area.id}`} key={area.id}><span><i className={`wd-dot ${area.color}`}/><b>{area.title}</b></span><small>{area._count.tasks + area._count.projects}</small></Link>)}</nav></aside>
      <section className="wd-workspace-detail">{selected ? <>
        <header className="wd-detail-head"><div className="wd-detail-title"><i className={`wd-dot ${selected.color}`}/><h2>{selected.title}</h2></div><details className="wd-more-menu"><summary aria-label={locale === "ko" ? "영역 메뉴" : "Area menu"}>•••</summary><div><EditAreaDialog area={{id:selected.id,title:selected.title,color:selected.color}} locale={locale}/><form action={archiveArea}><input type="hidden" name="areaId" value={selected.id}/><button>{locale === "ko" ? "보관" : "Archive"}</button></form><ConfirmSubmit action={deleteArea} fields={{ areaId: selected.id }} message={locale === "ko" ? "영역을 삭제할까요?" : "Delete this Area?"}><button className="dangerText">{locale === "ko" ? "삭제" : "Delete"}</button></ConfirmSubmit></div></details></header>
        ...
        <section className="wd-content-section"><div className="wd-content-head"><h3>{locale === "ko" ? "직접 작업" : "Direct tasks"}</h3><AreaTaskButton areaId={selected.id} locale={locale}/></div><div className="wd-area-tasks">{selected.tasks.map(task => {
          ...
          return <article className="wd-area-task" key={task.id}><div><strong>{task.title}</strong><span>{task.subtasks.length ? (locale === "ko" ? `하위 작업 ${task.subtasks.length}개` : `${task.subtasks.length} subtasks`) : (locale === "ko" ? "일정 없음" : "No schedule")}</span></div><div><TaskSchedulePicker taskId={task.id} locale={locale} compact scheduledItem={scheduledItem}/><details className="wd-more-menu"><summary>•••</summary><div><TaskEditDialog task={{ id:task.id,title:task.title,priority:task.priority,estimatedMinutes:task.estimatedMinutes,projectId:task.projectId,areaId:task.areaId,scheduledItem,repeat:task.recurrenceRule?.frequency ?? "none" }} projects={availableProjects} areas={areas.map(area => ({id:area.id,title:area.title,color:area.color}))} locale={locale}/><form action={archiveTask}><input type="hidden" name="taskId" value={task.id}/><button>{locale === "ko" ? "보관" : "Archive"}</button></form><ConfirmSubmit action={deleteTask} fields={{ taskId: task.id }} message={locale === "ko" ? `‘${task.title}’ 작업을 삭제할까요?` : `Delete “${task.title}”?`}><button className="dangerText">{locale === "ko" ? "삭제" : "Delete"}</button></ConfirmSubmit></div></details></div></article>;
