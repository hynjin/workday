...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ period?: string; start?: string; day?: string; difference?: string }> }) {
  ...
  const selectedAreaTotals = new Map<string, { name: string; seconds: number; color: string }>();
  selectedDay.sessions.forEach(session => {
    const key = reportIdentityKey(session.areaIdSnapshot, session.areaName, locale === "ko" ? "영역 없음" : "No Area");
    const current = selectedAreaTotals.get(key) ?? { name: session.areaName ?? (locale === "ko" ? "영역 없음" : "No Area"), seconds: 0, color: session.areaColor };
    current.seconds += session.durationSeconds ?? 0;
    selectedAreaTotals.set(key, current);
  });
  const reportPalette: Record<string,string> = { sky:"#78bde8", mint:"#86c9ac", lilac:"#c8b8df", peach:"#f7c8ad", butter:"#f3d991", gray:"#c4ccd1" };
  let donutCursor = 0;
  const donutStops = [...selectedAreaTotals.values()].map(row => {
    const start = donutCursor;
    donutCursor += selectedDay.seconds ? row.seconds / selectedDay.seconds * 100 : 0;
    return `${reportPalette[row.color] ?? reportPalette.gray} ${start}% ${donutCursor}%`;
  });
  const donutBackground = selectedDay.seconds ? `conic-gradient(${donutStops.join(",")})` : "#eef2f4";
  ...
  return <main className="wd-app"><AppNav/><section className="wd-main wd-report">
    ...
    <div className="reportControls"><nav className="reportSegments"><Link className={period === "week" ? "active" : ""} href="/growth?period=week">{locale === "ko" ? "주간" : "Weekly"}</Link><Link className={period === "month" ? "active" : ""} href="/growth?period=month">{locale === "ko" ? "월간" : "Monthly"}</Link></nav><nav className="weekNavigator"><Link className="button secondary" aria-label={locale === "ko" ? "이전 기간" : "Previous period"} href={`/growth?period=${period}&start=${dateKey(previousStart)}`}><WorkdayIcon name="chevronLeft" size={16}/></Link><strong>{rangeLabel}</strong>{canNext ? <Link className="button secondary" aria-label={locale === "ko" ? "다음 기간" : "Next period"} href={`/growth?period=${period}&start=${dateKey(selectedEnd)}`}><WorkdayIcon name="chevronRight" size={16}/></Link> : <span className="button secondary disabled" aria-hidden="true"><WorkdayIcon name="chevronRight" size={16}/></span>}</nav></div>

    <section className="reportTimeSummary" aria-label={locale === "ko" ? "집중 시간 요약" : "Focus time summary"}>
      <article><span>{period === "week" ? (locale === "ko" ? "주간 집중 목표" : "Weekly focus goal") : (locale === "ko" ? "월간 목표 전체" : "Monthly focus goal")}</span><strong>{formatDuration(periodGoalSeconds, false, locale)}</strong><small>{period === "week" ? `${goalProgress}%` : (locale === "ko" ? "주간 목표 합계" : "Sum of weekly goals")}</small></article>
    ...
    <section className="panel weeklyTrend reportTrend"><div className="sectionTitle"><h2>{period === "week" ? (locale === "ko" ? "날짜별 집중 흐름" : "Focus flow by date") : (locale === "ko" ? "월간 집중 기록" : "Monthly focus record")}</h2><span>{period === "week" ? (locale === "ko" ? "영역별 누적" : "Stacked by Area") : (locale === "ko" ? "기록된 날짜를 선택하세요" : "Select a recorded date")}</span></div><div className={`trendChart ${period === "month" ? "monthly" : ""}`}>{period === "month" && (locale === "ko" ? ["일","월","화","수","목","금","토"] : ["S","M","T","W","T","F","S"]).map((day,index) => <span className="reportMonthWeekday" key={`${day}-${index}`}>{day}</span>)}{dayRows.map((day,index) => {
    ...
    <div className="reportBreakdowns resultBreakdowns">
      <section className="panel reportRanking taskGoalBreakdown"><div className="reportRankingHead"><h2>{locale === "ko" ? "작업별 계획 · 실제 집중" : "Task plan · actual focus"}</h2><span>{differenceAscending ? (locale === "ko" ? "차이 작은 순" : "Smallest difference") : (locale === "ko" ? "차이 큰 순" : "Largest difference")} <Link className={differenceAscending ? "is-ascending" : ""} aria-label={locale === "ko" ? "정렬 순서 전환" : "Toggle sort order"} href={`${hrefFor(selectedDayKey)}&difference=${differenceAscending ? "large" : "small"}`}><WorkdayIcon name="sort" size={16}/></Link></span></div>{taskDifferenceRows.map(value => <div key={value.name}><span>{value.name}</span><strong>{formatDuration(value.goal, false, locale)} · {formatDuration(value.focused, false, locale)}</strong></div>)}</section>
