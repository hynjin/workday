...
export default async function Summary({ searchParams }: { searchParams: Promise<{ workdayId?: string }> }) {
  ...
  return <main className="shell narrow"><AppNav />
    ...
    {history.length > 0 && <section className="history"><h2>지난 작업일</h2><div>{history.map(day => <Link key={day.id} href={`/summary?workdayId=${day.id}`}>{formatWorkdayDate(day.workdayDate)}</Link>)}</div></section>}
